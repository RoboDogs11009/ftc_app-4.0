...
//end vu mark
   ...
   */
   ...
   public void rope (int counts, double timer){
       ...
       while (opModeIsActive() && (runtime.seconds() < timer)){
           telemetry.addData("Moving Rope", counts);
           telemetry.update();
       }
   ...
   public void arm (int counts, double timer){
...
//80.4937 counts per inch
    ...
    public void encoder (
        ...
        if (opModeIsActive()) {
                    ...
                    (runtime.seconds() < timeoutS) )
            {
                ...
                if (Rb.isBusy() == true)
                {
                    if ((newRightBackTarget - currPosRightBack) > 0) {
   ...
   public void liftEncoder (int counts, int ropeCounts, double timer){
        ...
        Li.setTargetPosition(-ropeCounts);
        Ra.setTargetPosition(counts);
        La.setTargetPosition(counts);
